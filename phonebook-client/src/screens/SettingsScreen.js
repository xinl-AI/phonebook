...
const SettingsScreen = () => {
  ...
  const submitPassword = async () => {
    if (!oldPassword) { setToastMsg('请输入原密码'); return; }
    if (!newPassword || newPassword.length < 6) { setToastMsg('新密码至少6位'); return; }
    if (newPassword !== confirmNew) { setToastMsg('两次密码输入不一致'); return; }
    ...
    if (result.code === 200) {
      ...
      setToastMsg(result.message || '修改失败');
    }
  };
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.section}>
          ...
          <SettingsItem label="清除缓存" onPress={handleClearCache} />
      ...
      {pwdModal && (
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {toastMsg !== '' && (
              <Text style={styles.errorText}>{toastMsg}</Text>
            )}
            ...
            <View style={styles.modalButtons}>
              ...
              <TouchableOpacity
                style={[styles.modalBtn, styles.modalConfirmBtn, pwdLoading && { opacity: 0.7 }]}
